...
class TwentyCrmBackend implements CrmBackend {
  ...
  async buscar(_contaId: string, _id: number): Promise<Oportunidade | null> {
    throw new Error('Twenty backend não configurado (NÃO VERIFICADO).');
  }
  async criar(): Promise<Oportunidade> {
    throw new Error('Twenty backend não configurado (NÃO VERIFICADO).');
  }
  async atualizar(_contaId: string, _id: number, _patch: Partial<OportunidadeInput>): Promise<Oportunidade | null> {
    throw new Error('Twenty backend não configurado (NÃO VERIFICADO).');
  }
  async buscarOwners(): Promise<{ id: string; nome: string }[]> {
    throw new Error('Twenty backend não configurado (NÃO VERIFICADO).');
  }
  async jaExistePorLead(_contaId: string, _leadId: number): Promise<boolean> {
    throw new Error('Twenty backend não configurado (NÃO VERIFICADO).');
  }
}
