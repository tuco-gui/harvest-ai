...
import { modulosDaConta } from '../../app/src/lib/autorizacao.ts';
...
class MemCrmBackend implements CrmBackend {
  ...
  async buscar(contaId: string, id: number) {
    const f = (this.por.get(contaId) ?? []).find((o) => o.id === id);
    return f ? { ...f } : null;
  }
  ...
  async atualizar(contaId: string, id: number, patch: Partial<OportunidadeInput>) {
    const lista = this.por.get(contaId) ?? [];
  ...
  async jaExistePorLead(contaId: string, leadId: number) {
    return (this.por.get(contaId) ?? []).some((o) => o.lead_id === leadId);
  }
}

async function main() {
  // --- Liberação comercial por conta ---
  assert(!modulosDaConta(['whatsapp'], 'admin').has('crm'), 'CRM fica oculto quando a conta não contratou');
  assert(modulosDaConta(['whatsapp', 'crm'], 'admin').has('crm'), 'CRM aparece quando a Figueira libera');
  assert(!modulosDaConta(['whatsapp'], 'super_admin').has('crm'), 'super admin testa a conta sem furar o bloqueio comercial');
  ...
  assert(await b.jaExistePorLead(CONTA_A, 10) === true, 'jaExistePorLead detecta vínculo na conta');
  assert(await b.jaExistePorLead(CONTA_B, 10) === false, 'mesmo lead não vaza para outra conta');
  assert(await b.jaExistePorLead(CONTA_A, 999) === false, 'jaExistePorLead ausente = false');

  // --- Mover estágio ---
  const movida = await b.atualizar(CONTA_A, op1.id, { estagio: 'proposta' });
  assert(movida?.estagio === 'proposta', 'mover estágio persiste');

  // --- Owner / valor / próxima ação ---
  const edit = await b.atualizar(CONTA_A, op1.id, { owner_id: 'u2', valor: 3200, proxima_acao: 'Enviar proposta' });
  ...
  const semCorromper = await b.atualizar(CONTA_A, op1.id, { estagio: 'estagio_falso' } as any);
  ...
  const buscada = await b.buscar(CONTA_A, op1.id);
  assert(buscada?.empresa === 'Padaria SP' && buscada?.valor === 3200, 'buscar reflete estado persistido');
  assert(await b.buscar(CONTA_B, op1.id) === null, 'buscar não acessa id de outra conta');
  assert(await b.atualizar(CONTA_B, op1.id, { valor: 9999 }) === null, 'atualizar não altera id de outra conta');
